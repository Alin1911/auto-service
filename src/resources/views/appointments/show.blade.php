        ...
        <div class="bg-white shadow-md rounded-lg p-6">
                ...
                <select name="vehicle_id" id="vehicle_id"
                    ...
                    @foreach (auth()->user()->vehicles as $vehicle)
                        <option value="{{ $vehicle->id }}"
                            {{ $vehicle->id == $appointment->vehicle_id ? 'selected' : '' }}>
            ...
            <div class="mb-4">
                ...
                <select name="appointment_type" id="appointment_type"
                    ...
                    <option value="Reparații" {{ $appointment->appointment_type == 'Reparații' ? 'selected' : '' }}>
                        Reparații</option>
                </select>
            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status"
                    class="mt-1 block w-full border-gray-300 rounded shadow-sm focus:ring focus:ring-indigo-200">
                    <option value="registered" {{ $appointment->status == 'registered' ? 'selected' : '' }}>
                        Înregistrată</option>
                    <option value="processed" {{ $appointment->status == 'processed' ? 'selected' : '' }}>Procesată
                    </option>
                    <option value="completed" {{ $appointment->status == 'completed' ? 'selected' : '' }}>Finalizată
                    </option>
                </select>
            </div>
        </div>
